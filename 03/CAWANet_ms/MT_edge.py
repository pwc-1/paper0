...
class VOCSegmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = img.resize((int(224), int(224)), Image.BILINEAR)
        target = target.resize((int(224), int(224)), Image.BILINEAR)


        if self.trian1 == True:
            target = target.resize((int(224), int(224)), Image.BILINEAR)
            # edg_mask = edg_mask.resize((int(512), int(512)), Image.BILINEAR)
        # if self.trian1 == False:
        #     img = img.resize((int(224), int(224)), Image.BILINEAR)
        #     target = target.resize((int(200), int(200)), Image.BILINEAR)
        # print(edg_mask)
        # if 'Oil' in self.images[index]:
        #     labels = torch.tensor(0)
        # elif 'Scr' in self.images[index]:
        ...
        return img, target, 1, edg_mask, size
